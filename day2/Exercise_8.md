...
Run this example with two GPUs using the `run-2gpus.sh` script:
...
`nvidia-smi` command. Check the `JOBID` of your running job with `squeue
...
`nvidia-smi`. A useful command to follow GPU usage is:

    watch nvidia-smi
    
It will update every 2 seconds. It should show values above 0% in the
GPU-Util column for all the GPUs you intend to use. Press Ctrl-C to exit
this view.
